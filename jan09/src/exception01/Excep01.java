 ...
 * 
 * 컴파일 -> 실행 -> 실행중 예외발생 -> VM이 발생된 예외의 종류 및 내용을 파악한 뒤 예외 객체를 생성함 -> 발생된 코드를 밖으로 던짐(Throw) ->
 * 예외의 콜 스택에 전이 -> main 메소드 밖으로 던지게 되면 프로그램 비정상 종료
 * 
 ...
 * 처리할 수 없습니다.
 * 
 * 다음 Exception이란 클래스의 하위 타입들이 바로 프로그래머가 처리해야할 예외입니다.
 * 따라서 보통 예외다 라고 표현할 때 최고 클래스는 Exception입니다.
 * 
 * Throwable클래스는 자식 으로 Error을 가지고 있기 떄문에 예외의 최고 클래스라고 하지 않습니다.
 * 
 * 예외의 종류
 * 크게 두 가지로 나뉩니다.
 * 
